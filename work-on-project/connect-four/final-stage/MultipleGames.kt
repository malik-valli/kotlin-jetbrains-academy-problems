...
const val DEFAULT_NUM_OF_GAMES = 1
...
const val Q_MULTIPLE_GAMES = "Do you want to play single or multiple games?\n" +
...
var numOfGames = DEFAULT_NUM_OF_GAMES
var currentGame = DEFAULT_NUM_OF_GAMES
...
fun setNumOfGames() {
    var input: String
    while (true) {
        println(Q_MULTIPLE_GAMES)
        input = readln()
        if (input.isEmpty() || input == DEFAULT_NUM_OF_GAMES.toString()) break
        try {
            check(input.toInt() >= 1)
...
fun play() {
    if (numOfGames == 1) println(SINGLE_GAME)
    else println("Game #$currentGame")

    drawBoard()

    var currentPlayer = if (currentGame % 2 == 0) secondPlayerName else firstPlayerName
